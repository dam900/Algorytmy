import 'package:quick_sort/node.dart';
...
class DoubleLinkedList<T> {
  ...
  void swapValue(int firstIndex, int secondIndex){
    int i = 0;
    int j = 0;
    if (firstIndex ==  secondIndex){
      return;
    }
    if (firstIndex > this._size || secondIndex > this._size){
      return;
    }
    Node<T>? tmpFirst = null;
    Node<T>? tmpSecond = null;
    Node<T>? currNode = _head;
    while (currNode != null) {
      if (i == firstIndex){
        tmpFirst = currNode;
